...
import crystalplan.model.tools
...
class FrameMain(wx.Frame):
    ...
    def _init_menuFile(self, parent):
        ...
        parent.Append(id, 'Save experiment to file...\tCtrl+S',
                    'Save the experiment to a .exp file so that it can be re-loaded later.', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuSave, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Load experiment from file...\tCtrl+L', 'Load a .exp file.', kind=wx.ITEM_NORMAL)
        ...
        if gui_utils.fourcircle_mode():
            ...
            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Load a HFIR .int file...\tCtrl+I', 
                          'Load a peaks file from HFIR software to compare predicted and real peaks.', kind=wx.ITEM_NORMAL)
            self.Bind(wx.EVT_MENU, self.OnMenuLoadIntegrateHFIR, id=id)

            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Load a HFIR UB matrix and lattice parameters file...\tCtrl+U', 
                          'Load a UB matrix file made by HFIR software, and a corresponding lattice parameters file.', kind=wx.ITEM_NORMAL)
            self.Bind(wx.EVT_MENU, self.OnMenuLoadHFIRUB, id=id)

        else:
            # ---------- ISAW loading menus --------------------
            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Load an older ISAW .integrate or .peaks file with sequential det. numbers', 
                        'Load a peaks file from ISAW to compare predicted and real peaks. Use this menu for older files (made with ISAW before ~April 2011).', kind=wx.ITEM_NORMAL)
            self.Bind(wx.EVT_MENU, self.OnMenuLoadIntegrateOld, id=id)

            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Load an ISAW .integrate or .peaks file...\tCtrl+I',
                        'Load a peaks file from ISAW to compare predicted and real peaks.', kind=wx.ITEM_NORMAL)
            self.Bind(wx.EVT_MENU, self.OnMenuLoadIntegrateNew, id=id)

            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Load an ISAW UB matrix (.mat) file...\tCtrl+U',
                        'Load a UB matrix file made by ISAW (goniometer-corrected; not by ISAWev).', kind=wx.ITEM_NORMAL)
            self.Bind(wx.EVT_MENU, self.OnMenuLoadUB, id=id)


            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Load a Lauegen .ldm UB matrix file...\tCtrl+M',
                        'Load a Lauegen .ldm file containing the crystal orientation for the IMAGINE instrument.', kind=wx.ITEM_NORMAL)
        ...
        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Save sample orientations to CSV file...\tCtrl+D',
                    'Make a CSV file containing the list of motor positions.', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuSaveToCSV, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Preferences...', 'Change preferences for calculations, display, and others.', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuPreferences, id=id)

        parent.AppendSeparator()

        parent.Append(wx.ID_EXIT, 'Quit\tCtrl+Q', 'Exit the program.', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuQuit, id=wx.ID_EXIT)

    # -------------------------------------------------------------------------
    def _init_menuView(self, parent):
        id = wx.NewIdRef(count=1)
        parent.Append(id, 'View Q-Space in 3D\tF2', 'Make a CSV file containing the list of motor positions.', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuView3D, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'New single reflection info window\tF3', 'Open a new window with info for a single HKL reflection.', kind=wx.ITEM_NORMAL)
    ...
    def _init_menuParameters(self, parent):
        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Other...', '', kind=wx.ITEM_NORMAL)
    ...
    def _init_menuTools(self, parent):
        ...
        if not gui_utils.fourcircle_mode():
            id = wx.NewIdRef(count=1)
            parent.Append(id, 'Automatic Coverage Optimizer...\tCtrl+O', '', kind=wx.ITEM_NORMAL)
            self.Bind(wx.EVT_MENU, self.OnMenuOptimizePositions, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Compare measured to predicted peak positions...', '', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuComparePeaks, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Find angles for all HKL.', '', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuFourCircleAllHKL, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Simple Laue Plots', '', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuLauePlot, id=id)


    def _init_menuHelp(self, parent):
        id = wx.NewIdRef(count=1)
        parent.Append(id, 'Open User Guide in WebBrowser\tF1', '', kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnMenuUserGuide, id=id)

        id = wx.NewIdRef(count=1)
        parent.Append(id, 'About %s...' % CrystalPlan_version.package_name, '', kind=wx.ITEM_NORMAL)
...
#        self.Bind(wx.EVT_MENU, self.OnMenuGenerateUserGuide, id=id)
    ...
    def load_HFIR_ubmatrix_file_dialog(self, parent):
        ...
        dialog = wx.FileDialog ( parent, defaultFile=filename, defaultDir=path, message='Load a HFIR UB Matrix file', wildcard=filters, style=wx.FD_OPEN )
        ...
        dialog = wx.FileDialog ( parent, defaultFile=load_filename, defaultDir=path, message='Load a HFIR lattice parameters file', wildcard=filters, style=wx.FD_OPEN )
        if dialog.ShowModal() == wx.ID_OK:
    ...
    def load_ubmatrix_file_dialog(self, parent):
        ...
        (path, filename) = os.path.split(self.last_ubmatrix_path)
        dialog = wx.FileDialog ( parent, defaultFile=filename, defaultDir=path, message='Load an ISAW (goniometer-corrected) .mat file', wildcard=filters, style=wx.FD_OPEN )
        if dialog.ShowModal() == wx.ID_OK:
            filename = dialog.GetPath()
            self.last_ubmatrix_path = filename
            dialog.Destroy()
    ...
    def load_ldm_file_dialog(self, parent):
        ...
        dialog = wx.FileDialog ( parent, defaultFile=filename, defaultDir=path, message='Load an Lauegen .ldm file', wildcard=filters, style=wx.FD_OPEN )
        if dialog.ShowModal() == wx.ID_OK:
            filename = dialog.GetPath()
...
#        return frame
    ...
    def _init_ctrls(self, prnt):
        ...
        self.statusBar_main = wx.StatusBar(id=wxID_FRAMEMAINSTATUSBAR_MAIN,
              ...
              style=wx.RESIZE_BORDER | wx.STB_SIZEGRIP)
