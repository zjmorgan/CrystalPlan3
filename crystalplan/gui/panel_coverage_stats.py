...
class PanelCoverageStats(wx.Panel):
    ...
    def show_stats(self, symmetry, coverage_pct, redundant_pct):
        ...
        self.gaugeCoverage.SetToolTip('Gauge showing the % covered.')
        self.gaugeRedundancy.SetValue(redundant_pct)
        self.gaugeRedundancy.SetToolTip('Gauge showing the % measured more than once (redundancy).')
    ...
    def show_reflection_stats(self, use_symmetry, stats):
        ...
        self.staticTextStatsRedundant.SetLabel("%5.1f%% measured > once:" % redundant_pct)
        self.gaugeCoverage.SetValue(coverage_pct)
        self.gaugeCoverage.SetToolTip('%d reflections were measured, out of %d' % (stats.measured, stats.total))
        self.gaugeRedundancy.SetValue(redundant_pct)
        self.gaugeRedundancy.SetToolTip('%d reflections were measured more than once, out of %d' % (stats.redundant, stats.total))
        #Do we see the count of reflections?
