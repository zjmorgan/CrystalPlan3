...
class SliceControl(wx.PyControl):
    ...
    def __init__(self, parent, use_slice=False, apply_slice_method=None,
        ...
        wx.Control.__init__(self, parent, id, pos, size, wx.TAB_TRAVERSAL, validator, name)
...
##                self._last_slice = (self.slice_min, self.slice_max)
    ...
    def DrawTick(self, dc, x, y, label, horizontal=True):
        ...
        if horizontal:
            ...
            dc.DrawLabel(label,
                wx.Rect( 0, 0, int(px), int(py+tick+1)),
                alignment=wx.ALIGN_CENTER_HORIZONTAL|wx.ALIGN_TOP )
        else:
            dc.DrawLine( px, py, px-tick-1, py)
            dc.DrawLabel(label,
                wx.Rect(0, 0, int(self.GetX(x)-tick-1), int(self.GetY(y))),
    ...
    def Draw(self, dc):
        ...
        dc.DrawLabel('%' , wx.Rect( 0, 0, int(self.GetX(self.data_x[0])-2), int(self.GetY(yrange/2))), alignment=wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL )
        ...
        if self.energy_mode:
            ...
            if d_mode:
                ...
                for x_label in range(0, int(xrange+1), steps):
                    self.DrawTick(dc, x_label, 0, ("%d" % x_label), horizontal=True)
                #Label
                dc.DrawLabel('q' , wx.Rect(0, 0, 2, int(height)), alignment=wx.ALIGN_LEFT|wx.ALIGN_BOTTOM )
