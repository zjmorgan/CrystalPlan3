...
def dialog_to_save_experiment_to_CSV(parent):
    ...
    dialog = wx.FileDialog ( parent, defaultFile=filename, defaultDir=path, message='Save the experiment plan to CSV file', wildcard=filters, style=wx.FD_SAVE )
...
def save_experiment_file_dialog(parent):
    ...
    dialog = wx.FileDialog ( parent, defaultFile=filename, defaultDir=path, message='Save the experiment plan to EXP file', wildcard=filters, style=wx.FD_SAVE )
    if dialog.ShowModal() == wx.ID_OK:
        filename = dialog.GetPath()
