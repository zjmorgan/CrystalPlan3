...
def getq_code(elevation, az, rot_matrix, wl_input, wl_output):
...
def getq_inelastic_code(elevation, az, rot_matrix, wl_input, wl_output):
    # Assuming azimuth of zero points to z positive = same direction as incident radiation.
    r2 = cos(elevation) / wl_output
    z=cos(az) * r2
    x=sin(az) * r2
...
def getq(az, elevation, wl_output, q_rot_matrix, wl_input=None):
    ...
    if wl_input is None:
        ...
        q = getq_code(elevation, az, rot_matrix, wl_input, wl_output)
        ...
        q_both = getq_inelastic_code(elevation, az, rot_matrix, wl_input, wl_output)
