...
from crystalplan.gui import mlab_utils
...
from crystalplan.model import compiled
...
class LimitedGoniometer(Goniometer):
    ...
    def get_fitness_function_c_code(self):
        ...
        angles = np.array(args)

        return compiled.fit_fun3, angles
    ...
    def _angle_fitness_brute(self, rot_angle_list, initial_rotation_matrix, ending_vec, starting_vec):
        ...
        #List of fitnesses
        fitnesses = np.zeros(3*rot_angle_list[0])
        chi_list = np.zeros(3*rot_angle_list[0])
        phi_list = np.zeros(3*rot_angle_list[0])
        omega_list = np.zeros(3*rot_angle_list[0])
        ...
        # [rot_angle_list, ending_vec, initial_rotation_matrix, fitnesses, chi_list, phi_list, omega_list]
        # ret = weave.inline(code, varlist, compiler='gcc', support_code=support)
        
        fitness_function, angles = self.get_fitness_function_c_code()
        compiled._ang_fit_brute(rot_angle_list, ending_vec, initial_rotation_matrix, fitnesses, chi_list, phi_list, omega_list, fitness_function, angles)
    ...
    def calculate_angles_to_rotate_vector(self, starting_vec, ending_vec, starting_angles=None, search_method=0):
        ...
        if search_method:
            ...
            step = np.deg2rad(2)
            (best_rot_angle, best_angles) = optimize(-2.2*pi, pi*2.2, step)
            ...
            step = np.deg2rad(2)
            # (best_rot_angle, best_angles) = optimize(-0.2*pi, pi*2.2, step)
            (best_rot_angle, best_angles) = optimize(-1.2*pi, pi*1.2, step)
            for x in range(4):
                newstep = step/10
                (best_rot_angle, best_angles) = optimize(best_rot_angle-step, best_rot_angle+step, newstep)
                step = newstep
...
class SNAPLimitedGoniometer(LimitedGoniometer):
    ...
    def get_fitness_function_c_code(self):
        ...
        return compiled.fit_fun2, angles
...
class MandiVaryOmegaGoniometer(LimitedGoniometer):
    ...
    def get_fitness_function_c_code(self):
        ...
        return compiled.fit_fun_VARY, angles
...
#        self.leg_safe_zone[2, :, :center] = False
        ...
        if visualize:
            plt.figure()
            for leg in range(3):
                plt.pcolor(self.leg_safe_xaxis+fixed_plate_zero[COORD_X, leg], self.leg_safe_zaxis+fixed_plate_zero[COORD_Z, leg], self.leg_safe_zone[leg, :, :].transpose())
            plt.xlabel("x")
            plt.ylabel("z")
            plt.title("Allowable XZ leg positions for the 3 legs.")
            plt.draw()
            plt.axis('equal')
            #pylab.show()
    ...
    def calculate_allowable_angles(self, angular_resolution=0.01, visualize=0, optimized=False):
        ...
        if visualize==1:
            ...
            for i_phi in range(self.phi_list.size):
                ...
                if visualize:
                    plt.figure()
                    plt.imshow(self.allowed[i_phi, :, :], interpolation='nearest', extent=(-self.omega_range, self.omega_range, -self.chi_range, self.chi_range))
                    plt.xlabel("Omega")
                    plt.ylabel("Chi")
                    plt.title("Allowable angles for phi=%s" % phi)
                    plt.draw()
                    time.sleep(0.2)
                    plt.draw()
            time.sleep(1)
    ...
    def plot_goniometer(self, first_plot=False):
        ...
        if abs(error) > 0.01:
            warnings.warn('The middle of the sample plate\'s 3 points does not match the middle calculated with sample motor rotation!\nError is %f' % difference)

        if vector_length(self.pin) > 0.01:
            warnings.warn('The calculated sample position is not zero after rotation/translation! This means that the calculation failed to keep the sample centered, somehow. Sample is at: '+str(self.pin.cT))

        error = vector_length(self.motor - self.sample_middle) - self.motor_vector_length
        if abs(error) > 0.01:
            warnings.warn('The calculated length of the motor vector after rotation does not match the expected length! Error is '+str(error))

        #---- Start plotting -----
        if first_plot:
            mlab.clf()
            mlab_utils.draw_cartesian_axes(20, np.array([0, 150, 0]))
            #Plot The "zero" position sample plate
            mlab_utils.plot3dpatch(self.sample_plate_zero, (0, 1, 1), 0.5)
            #Show the range of movement of the legs on the fixed plate
            mlab_utils.points3d(self.fixed_plate_zero, scale_factor=7)
            mlab_utils.text3d(self.fixed_plate_zero[:, MOUNT_A], 'A', color=(0,0,0))
            mlab_utils.text3d(self.fixed_plate_zero[:, MOUNT_B], 'B', color=(0,0,0))
            mlab_utils.text3d(self.fixed_plate_zero[:, MOUNT_C], 'C', color=(0,0,0))
...
#                mlab.imshow(imagedata, extent=extent)
        ...
        self._plot_sample_plate = mlab_utils.plot3dpatch(self.sample_plate, (1,0,0), 1, plotobj=self._plot_sample_plate)

        #Draw the fixed plate position
        self._plot_fixed_plate = mlab_utils.plot3dpatch(self.fixed_plate, (0,1,0), 0.5, plotobj=self._plot_fixed_plate)
        self._plot_leg_points = mlab_utils.plot3dpoints(self.fixed_plate, color=(0,1,0), plotobj=self._plot_leg_points)
...
#            if self.leg_fault[leg]: color = (1,0,0)
            self._plot_legs[leg] = mlab_utils.arrow(self.sample_plate[:, leg], self.fixed_plate[:, leg], color=color, plotobj=self._plot_legs[leg])

        #Arrow for the sample pin
        self._plot_pin = mlab_utils.arrow(self.sample_middle, self.pin, plotobj=self._plot_pin)
        #Arrow for motor direction
        color = (0, 0.5, 0)
        self._plot_motor = mlab_utils.arrow(self.sample_middle, self.motor, color=color, plotobj=self._plot_motor)
        #Point in the middle
...
#        return
    ...
    plt.title("Allowable XZ sample positions")
    for (iy, y) in enumerate(positions):
        print("At y of", y, ", # of points = ", np.sum( allowed[:, iy,:]))
        if iy < 16:
            plt.subplot(4,4,iy+1)
            plt.pcolor(positions, positions, allowed[:, iy, :].transpose())
            plt.xlabel("x")
            plt.ylabel("z")
            plt.title("y = %.3f mm" % y)
            plt.draw()
...
def initialize_goniometers():
    ...
    goniometers.append( HB3AGoniometer() )
