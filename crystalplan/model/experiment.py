...
class ParamsDict(dict):
    """Subclass of dictionary used to store the display parameters.
...
class Experiment:
    ...
    def initialize_volume_symmetry_map(self):
        ...
        for rd in range(order):
            ...
            q_equiv[:,:, rd] =  np.dot(B,   np.dot(pg.table[rd], hkl) )

            #Now we need to find the index into the array.
            #Start by finding the x,y,z, indices
            ix = numpy_utils.index_array_evenly_spaced(-qlim, n, inst.q_resolution, q_equiv[0, :, rd])
            iy = numpy_utils.index_array_evenly_spaced(-qlim, n, inst.q_resolution, q_equiv[1, :, rd])
            iz = numpy_utils.index_array_evenly_spaced(-qlim, n, inst.q_resolution, q_equiv[2, :, rd])
            ...
            compiled.init_vol_symm_map(B, invB, symm, qres, qlim, n, order, table)
    ...
    def apply_volume_symmetry(self, use_inline_c=True):
        ...
        if use_inline_c and not config.cfg.force_pure_python:
            ...
            compiled.app_vol_symm(old_q, qspace_flat, numpix, order, symm)
            ...
            old_q = self.qspace * 1.0
            new_q = self.qspace * 0.0
        ...
        if self.qspace_displayed is None:
            ...
            if self.qspace is None:
                if self.inst is None:
                    ...
                    self.qspace = self.inst.make_blank_qspace(np.double)
    ...
    def calculate_coverage_stats(self):
        ...
        if config.cfg.force_pure_python:
            ...
            for i in range(num):
                ...
                layer = (self.qspace * bool_array)
                for k in range(3):
                    #Calculate the coverage where you measure the point more than i times.
                    covstat.coverage[k] = 100.0 * np.sum( (layer > k) ) / points_in_slice
            ...
            qspace = self.qspace.flatten()*1.
            qspace_size = qspace.size
            
            #The function returns a tuple
            (overall_points, overall_covered_points, overall_redundant_points) = compiled.calc_cov_stats(qspace, qspace_radius, q_step, qlim, total_points, qspace_size, num, covered_points0, covered_points1, covered_points2, covered_points3)
