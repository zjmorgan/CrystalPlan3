...
class FlatDetector(Detector):
    ...
    def get_detector_coordinates(self, beam, wl_min=0.0, wl_max=1e6):
        ...
        base_point = getattr(self, 'base_point').flatten()
        horizontal = getattr(self, 'horizontal').flatten()
        vertical = getattr(self, 'vertical').flatten()
        normal = getattr(self, 'normal').flatten()
        width = self.width
        height = self.height

        #Run the code
        error_count = compiled.det_coord(base_point, horizontal, vertical, normal, h_out, v_out, wl_out, distance_out, hits_it, beam, array_size, n_dot_base, height, width, wl_min, wl_max)
